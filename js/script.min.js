...
function status(o){return o.status>=200&&o.status<300?(console.log("Headers: %c "+o.headers.get("Content-Type")+" ","background-color: #222; color: #7bd235"),console.log("Status: %c "+o.status+" ","background-color: #222; color: #7bd235"),console.log("StatusText: %c "+o.statusText+" ","background-color: #222; color: #7bd235"),console.log("ResponseType: %c "+o.type+" ","background-color: #222; color: #7bd235"),console.log("ResponseUrl:  "+o.url),Promise.resolve(o)):Promise.reject(new Error(o.statusText))}function json(o){return o.json()}function displayData(o){const e=o.reduce((o,e)=>(o[e.code]=e.population,o),{}),t=Math.max(...Object.values(e));fetch("./img/russia.svg").then(o=>o.text()).then(o=>{let n=o.indexOf("<svg");n=n>=0?n:0;const s=SVG(o.slice(n)).addTo("#map").size("100%","100%").panZoom({wheelZoom:!0,zoomMin:1,zoomMax:4});for(const o of s.find("path")){const n=e[o.id()];isFinite(n)&&o.fill(getColor(n/t)),o.on("mouseover",()=>{popperEl.innerText=`${o.attr("name")}: ${n}`,popperEl.style.visibility="visible",popperInstance=Popper.createPopper(o.node,popperEl,{placement:"bottom"})}),o.on("mouseleave",()=>{popperEl.style.visibility="hidden"})}})}document.onreadystatechange=function(){"complete"==document.readyState&&(console.group("Status:"),console.log("Loading: %c%s","padding: 2px 5px; background-color: #7bd235; color: #fff",document.readyState),console.groupEnd())};const popperEl=document.getElementById("popper");let popperInstance;const getHex=o=>o.toString(16).padStart(2,"0"),getColor=o=>{const e="f50707",t="ffffff";if(!isFinite(o))return"#"+e;const n=Math.ceil(parseInt(e.substring(0,2),16)*o+parseInt(t.substring(0,2),16)*(1-o)),s=Math.ceil(parseInt(e.substring(2,4),16)*o+parseInt(t.substring(2,4),16)*(1-o)),r=Math.ceil(parseInt(e.substring(4,6),16)*o+parseInt(t.substring(4,6),16)*(1-o));return"#"+getHex(n)+getHex(s)+getHex(r)},url="./data/ru.json";fetch(url).then(status).then(json).then(function(o){console.log("%cЗапрос выполнен","padding: 2px 5px; background-color: #7bd235; color: #fff"),console.log("Количество объектов JSON: %c "+o.length+" ","background-color: #222; color: #7bd235"),displayData(o)}).catch(function(o){console.log("%c Ошибка:  "+o+" ","background-color: #f00; color: #fff",o)});